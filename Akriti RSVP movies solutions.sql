...
SELECT COUNT(*) as income_null FROM movie WHERE worlwide_gross_income IS NULL;
SELECT COUNT(*) as language_null FROM movie WHERE languages IS NULL;
SELECT COUNT(*) as production_null FROM movie WHERE production_company IS NULL;
...
SELECT  COUNT(*) as number_of_movies FROM movie WHERE year = 2019  
...
ON r.movie_id = m.id WHERE country = 'Germany' 
OR country = 'Italy' GROUP BY country;
...
WITH top_genre AS
(
SELECT g.genre, COUNT(g.movie_id) as movie_count FROM genre g INNER JOIN ratings r
ON g.movie_id = r.movie_id WHERE avg_rating>8 GROUP BY genre
...
top_genre WHERE r.avg_rating > 8 AND g.genre IN (top_genre.genre)
...
FROM role_mapping rm INNER JOIN names n ON n.id = rm.name_id
...
OVER(ORDER BY SUM(total_votes) DESC) AS prod_comp_rank FROM movie m
INNER JOIN ratings r ON m.id = r.movie_id GROUP BY production_company
...
ROUND(SUM(r.avg_rating*r.total_votes)/SUM(r.total_votes),2) AS actor_avg_rating FROM
names AS n INNER JOIN role_mapping AS a ON n.id=a.name_id INNER JOIN movie AS m
ON a.movie_id = m.id INNER JOIN ratings AS r ON m.id=r.movie_id
...
ROUND(SUM(r.avg_rating*r.total_votes)/SUM(r.total_votes),2) AS actress_avg_rating FROM
names AS n INNER JOIN role_mapping AS a ON n.id=a.name_id INNER JOIN movie AS m
ON a.movie_id = m.id INNER JOIN ratings AS r ON m.id=r.movie_id
...
SELECT
	...
	DENSE_RANK() OVER(PARTITION BY year ORDER BY CONVERT(REPLACE(TRIM(worlwide_gross_income), "$ ",""), 
